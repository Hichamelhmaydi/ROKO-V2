...
public interface ReservationRepository extends JpaRepository<Reservations, Long> {
    ...
    @Query("SELECT r FROM Reservations r WHERE r.user.id = :userId ORDER BY r.dateReservation DESC")
    ...
    @Query("SELECT COUNT(r) FROM Reservations r WHERE r.user.id = :userId")
    ...
    @Query("SELECT r FROM Reservations r WHERE r.user.id = :userId AND r.statut = :statut ORDER BY r.dateReservation DESC")
    ...
    @Query("SELECT COUNT(r) > 0 FROM Reservations r WHERE r.user.id = :userId AND r.voyage.id = :voyageId")
